...
import { AdvancedDynamicTexture, Rectangle, Button, TextBlock, StackPanel, Control, Checkbox } from '@babylonjs/gui';
...
class UnifiedSceneManager {
    ...
    setMaterial(target, materialType) {
    ...
    getLoadedModels() { return Array.from(this.loadedModels.keys()); }
    dispose() {
...
const UnifiedBabylonWorkspace = ({ workspaceId, isAdmin = false }) => {
    ...
    const setupPerformanceMonitoring = (manager) => {
    ...
    const handleMaterialChange = useCallback((materialId) => {
        if (sceneManager && selectedObjects.length > 0) {
            selectedObjects.forEach(objectId => {
                sceneManager.setMaterial(objectId, materialId);
            });
