...
export const useFeatureToggle = (featureStates, setFeatureStates) => {
    ...
    const toggleMultipleFeatures = useCallback((featureIds, enabled) => {
        const stateUpdates = {};
    ...
    const toggleCategoryFeatures = useCallback((category, enabled, features) => {
    ...
    const getFeaturesByCategory = useCallback((category, features) => {
        return features.filter(feature => feature.category === category);
    }, []);
    // Check if any feature in a category is active
    const isCategoryActive = useCallback((category, features) => {
    ...
    const getCategoryActiveCount = useCallback((category, features) => {
        const categoryFeatures = getFeaturesByCategory(category, features);
