...
const ErgonomicTesting = ({ scene, onAvatarChange }) => {
    const [selectedAvatar, setSelectedAvatar] = useState('adult');
    const [avatarPosition, setAvatarPosition] = useState(new BABYLON.Vector3(0, 0, 0));
    const [showReachZone, setShowReachZone] = useState(true);
    const [showClearanceZone, setShowClearanceZone] = useState(true);
    const [isActive, setIsActive] = useState(false);
    const avatarMeshRef = useRef(null);
    const reachZoneRef = useRef(null);
    const clearanceZoneRef = useRef(null);
    ...
    const createAvatarMesh = (type) => {
        ...
        body.material.diffuseColor = spec.color;
        // Head (sphere)
        ...
        head.material.diffuseColor = spec.color;
        ...
        leftArm.material.diffuseColor = spec.color;
        ...
        rightArm.material.diffuseColor = spec.color;
        ...
        if (type === 'wheelchair') {
            ...
            wheelchairBase.material.diffuseColor = new BABYLON.Color3(0.3, 0.3, 0.3);
            // Wheels
            ...
            wheel1.material.diffuseColor = new BABYLON.Color3(0.1, 0.1, 0.1);
            ...
            wheel2.material.diffuseColor = new BABYLON.Color3(0.1, 0.1, 0.1);
            body.parent = avatar;
            head.parent = avatar;
    ...
    const createReachZone = (avatar, spec) => {
        ...
        reachZone.material.diffuseColor = new BABYLON.Color3(0.2, 0.8, 0.2);
        reachZone.material.alpha = 0.3;
        reachZone.material.wireframe = true;
        return reachZone;
    };
    // Create clearance zone
    const createClearanceZone = (avatar, spec) => {
        ...
        clearanceZone.material.diffuseColor = new BABYLON.Color3(0.8, 0.2, 0.2);
        clearanceZone.material.alpha = 0.2;
        clearanceZone.material.wireframe = true;
        return clearanceZone;
    };
    // Analyze conflicts
    const analyzeConflicts = (avatar, spec) => {
        const conflicts = [];
    ...
    const createConflictVisualization = (conflicts) => {
    ...
    const updateAvatar = () => {
        ...
        if (avatarMeshRef.current)
            avatarMeshRef.current.dispose();
        if (reachZoneRef.current)
            reachZoneRef.current.dispose();
        if (clearanceZoneRef.current)
            clearanceZoneRef.current.dispose();
        // Create new avatar
    ...
    return (_jsxs("div", { style: {
            ...
            borderRadius: '8px',
            color: '#f1f5f9'
        }, children: [_jsx("h3", { style: { margin: '0 0 16px 0', fontSize: '16px' }, children: "Ergonomic Testing" }), _jsx("div", { style: { marginBottom: '16px' }, children: _jsx("button", { onClick: () => setIsActive(!isActive), style: {
                        padding: '8px 16px',
                        ...
                        fontSize: '14px',
                        cursor: 'pointer'
                    }, children: isActive ? 'Stop Testing' : 'Start Testing' }) }), _jsxs("div", { style: { marginBottom: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 8px 0', fontSize: '14px' }, children: "Avatar Type" }), _jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '4px' }, children: Object.keys(avatarSpecs).map((type) => (_jsx("button", { onClick: () => setSelectedAvatar(type), style: {
                                padding: '6px',
                                ...
                                fontSize: '12px',
                                cursor: 'pointer'
                            }, children: type.charAt(0).toUpperCase() + type.slice(1) }, type))) })] }), _jsxs("div", { style: { borderTop: '1px solid #334155', paddingTop: '16px', marginBottom: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 12px 0', fontSize: '14px' }, children: "Visualization" }), _jsx("div", { style: { marginBottom: '8px' }, children: _jsxs("label", { style: { display: 'block', marginBottom: '4px', fontSize: '12px' }, children: [_jsx("input", { type: "checkbox", checked: showReachZone, onChange: (e) => setShowReachZone(e.target.checked) }), "Show Reach Zone (Green)"] }) }), _jsx("div", { style: { marginBottom: '8px' }, children: _jsxs("label", { style: { display: 'block', marginBottom: '4px', fontSize: '12px' }, children: [_jsx("input", { type: "checkbox", checked: showClearanceZone, onChange: (e) => setShowClearanceZone(e.target.checked) }), "Show Clearance Zone (Red)"] }) })] }), _jsxs("div", { style: { borderTop: '1px solid #334155', paddingTop: '16px', marginBottom: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 8px 0', fontSize: '14px' }, children: "Specifications" }), _jsxs("div", { style: { fontSize: '12px', color: '#94a3b8' }, children: [_jsxs("div", { children: ["Height: ", spec.height.toFixed(2), "m"] }), _jsxs("div", { children: ["Reach: ", spec.reach.toFixed(2), "m"] }), _jsxs("div", { children: ["Eye Height: ", spec.eyeHeight.toFixed(2), "m"] }), _jsxs("div", { children: ["Shoulder Width: ", spec.shoulderWidth.toFixed(2), "m"] })] })] }), _jsxs("div", { style: { borderTop: '1px solid #334155', paddingTop: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 8px 0', fontSize: '14px' }, children: "Instructions" }), _jsx("div", { style: { fontSize: '12px', color: '#94a3b8', marginBottom: '8px' }, children: isActive ? 'Click on the floor to position the avatar.' : 'Start testing to place avatars.' }), _jsx("div", { style: { fontSize: '12px', color: '#94a3b8' }, children: "Red spheres indicate conflicts, yellow for warnings, green for optimal conditions." })] })] }));
};
