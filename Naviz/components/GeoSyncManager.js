import { TransformNode, Vector3, MeshBuilder, StandardMaterial, Color3 } from '@babylonjs/core';
export class GeoSyncManager {
     ...
     */
    async updateObjectPosition(objectId, position, rotation) {
        const object = this.objects.get(objectId);
        if (!object) {
            return false;
        }
     ...
     */
    async syncObjectFromGeoLocation(objectId) {
        const object = this.objects.get(objectId);
        if (!object) {
            return false;
        }
     ...
     */
    geoLocationToPosition(geoLocation) {
     ...
     */
    positionToGeoLocation(position) {
     ...
     */
    createWorkspaceArea(name, bounds, elevation) {
     ...
     */
    addMapOverlay(workspaceId, imageUrl) {
        const workspace = this.workspaces.get(workspaceId);
        if (!workspace) {
            return false;
        }
    ...
    createObjectNode(object) {
        ...
        sphere.material.diffuseColor = Color3.Green();
    ...
    async deleteRemoteObject(objectId) {
        try {
            // TODO: Implement actual API call
