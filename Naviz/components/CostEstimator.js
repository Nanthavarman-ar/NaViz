...
export class CostEstimator {
    constructor(bimManager, simulationManager) {
        ...
        Object.defineProperty(this, "materialDatabase", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "laborDatabase", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "costEstimates", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        Object.defineProperty(this, "clientBudget", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "budgetEnabled", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "vendorDatabase", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "vendorAvailability", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        }); // materialId -> availability data
        Object.defineProperty(this, "supplyChainLinks", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        }); // materialId -> supply chain data
        Object.defineProperty(this, "priceUpdateInterval", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 3600000
    ...
    getBudgetFilteredMaterials(materialType) {
    ...
    calculateProjectCostWithBudget(modelId, region = this.defaultRegion) {
        if (!this.budgetEnabled) {
    ...
    getBudgetRecommendations(originalMaterialId, quantity, region = this.defaultRegion) {
        if (!this.budgetEnabled) {
        ...
        if (!originalMaterial)
            return null;
        const originalEstimate = this.calculateElementCost({ id: 'temp', name: 'temp', type: 'wall', category: 'Architecture', position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, scale: { x: 1, y: 1, z: 1 }, material: originalMaterialId, properties: {}, visible: true }, quantity, region);
        if (!originalEstimate)
            return null;
        ...
        const budgetFilteredAlternatives = alternativeMaterials.filter(altId => {
            ...
            if (!altMaterial)
                return false;
            const altEstimate = this.calculateElementCost({ id: 'temp', name: 'temp', type: 'wall', category: 'Architecture', position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, scale: { x: 1, y: 1, z: 1 }, material: altId, properties: {}, visible: true }, quantity, region);
        ...
        const bestAlternative = comparisonAlternatives.length > 0
            ? comparisonAlternatives.reduce((best, current) => current.costSavings > best.costSavings ? current : best).materialId
            : '';
        return {
    ...
    generateBudgetRecommendation(original, alternative, costSavings, alternativeCost) {
    ...
    calculateElementCost(element, quantity, region = this.defaultRegion, supplier) {
    ...
    calculateCostBreakdown(material, quantity, region, supplier) {
    ...
    compareMaterialCosts(originalMaterialId, quantity, region = this.defaultRegion, alternatives) {
        const originalMaterial = this.materialDatabase.get(originalMaterialId);
        if (!originalMaterial)
            return null;
        const originalEstimate = this.calculateElementCost({ id: 'temp', name: 'temp', type: 'wall', category: 'Architecture', position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, scale: { x: 1, y: 1, z: 1 }, material: originalMaterialId, properties: {}, visible: true }, quantity, region);
    ...
    getAlternativeMaterials(materialType) {
        const alternatives = {
    ...
    generateRecommendation(original, alternative, costSavings) {
        const savingsPercent = (costSavings / (original.baseCostPerUnit * 1.5)) * 100;
        if (savingsPercent > 15) {
    ...
    getEnvironmentalImpact(material) {
        const impactScores = {
    ...
    calculateProjectCost(modelId, region = this.defaultRegion) {
        const model = this.bimManager.getModelById(modelId);
        if (!model)
            return null;
        let totalMaterials = 0;
    ...
    invalidateEstimatesForMaterial(materialId) {
    ...
    getMaterialCostData(materialId) {
    ...
    exportCostData(modelId, format = 'json') {
    ...
    estimateMaterialLifecycleCost(material) {
        // Get material cost data
    ...
    recordPriceHistory(materialId, vendorId, price, quantity) {
    ...
    addSupplyChainAlert(materialId, type, message, severity = 'medium') {
        if (!this.supplyChainLinks.has(materialId)) {
            this.supplyChainLinks.set(materialId, {
                materialId,
                preferredVendors: [],
    ...
    getVendorsForMaterial(materialId) {
        const vendors = [];
    ...
    calculateCostBreakdownWithVendor(material, quantity, vendorPrice, region) {
    ...
    getBestVendor(materialId, quantity) {
    ...
    getSupplyChainAlerts(materialId) {
        if (materialId) {
            const supplyChainLink = this.supplyChainLinks.get(materialId);
    ...
    exportSupplyChainData(materialId, format = 'json') {
        if (materialId) {
            const supplyChainLink = this.supplyChainLinks.get(materialId);
            if (!supplyChainLink)
                return '';
        ...
        if (format === 'csv') {
            ...
            Object.values(allData).forEach((link) => {
    ...
    swapMaterialWithPerformanceImpact(element, newMaterialId, quantity, region = this.defaultRegion) {
        ...
        const originalMaterialForLifecycle = {
            id: originalMaterialData.id,
            name: originalMaterialData.name,
            type: originalMaterialData.type,
        ...
        const performanceImpact = this.calculatePerformanceImpact(originalMaterialData, newMaterialData, quantity, originalEstimate.costBreakdown.total, newEstimate.costBreakdown.total);
        // Generate recommendations and warnings
        const recommendations = this.generateSwapRecommendations(originalMaterialData, newMaterialData, performanceImpact);
        const warnings = this.generateSwapWarnings(originalMaterialData, newMaterialData, performanceImpact);
        return {
            originalEstimate,
            newEstimate,
            performanceImpact,
    ...
    calculatePerformanceImpact(originalMaterial, newMaterial, quantity, originalCost, newCost) {
        ...
        const originalLifecycleCost = this.estimateMaterialLifecycleCost({
            ...
            recycled: false,
            renewable: false,
            location: { x: 0, y: 0, z: 0 }, // Vector3 from Babylon.js
            volume: 100
        });
    ...
    getInsulationProperties(material) {
        const insulationMap = {
    ...
    getStructuralProperties(material) {
        const structuralMap = {
    ...
    getEnvironmentalProperties(material) {
    ...
    calculateOverallScore(material, cost, lifecycleCost) {
        ...
        const score = (environmentalScore * 0.3 +
            durabilityScore * 0.3 +
            energyScore * 0.2 +
    ...
    getOverallRecommendation(scoreImprovement, costDifference) {
    ...
    generateSwapRecommendations(originalMaterial, newMaterial, impact) {
    ...
    generateSwapWarnings(originalMaterial, newMaterial, impact) {
        const warnings = [];
