...
const SoundPrivacySimulation = ({ scene, onPrivacyUpdate }) => {
    const [soundSources, setSoundSources] = useState([]);
    const [selectedSourceType, setSelectedSourceType] = useState('conversation');
    const [isActive, setIsActive] = useState(false);
    const [showSoundWaves, setShowSoundWaves] = useState(true);
    const [showPrivacyZones, setShowPrivacyZones] = useState(true);
    const [showLeakagePaths, setShowLeakagePaths] = useState(true);
    const [privacyThreshold, setPrivacyThreshold] = useState(30); // dB
    const soundMeshesRef = useRef([]);
    ...
    const createSoundSourceMesh = (source) => {
    ...
    const createPrivacyZones = () => {
        const zones = [];
        // Detect rooms based on mesh arrangement
        const roomMeshes = scene.meshes.filter(mesh => mesh.name.toLowerCase().includes('wall') ||
    ...
    const visualizePrivacyZones = (zones) => {
    ...
    const calculateSoundPropagation = (sources, zones) => {
        ...
        sources.forEach(source => {
            updatedZones.forEach(zone => {
                ...
                const barriers = scene.meshes.filter(mesh => mesh.name.toLowerCase().includes('wall') &&
    ...
    const isBetweenPoints = (point, start, end) => {
    ...
    const visualizeLeakagePaths = (paths) => {
    ...
    // Remove sound source
    const removeSoundSource = (sourceId) => {
    ...
    // Generate recommendations
    const generateRecommendations = (zones, paths) => {
    ...
    return (_jsxs("div", { className: "sound-privacy-panel", children: [_jsx("h3", { children: "Sound Privacy Simulation" }), _jsx("div", { className: "simulation-toggle", children: _jsx("button", { onClick: toggleSoundSimulation, className: `toggle-button ${isActive ? 'active' : 'inactive'}`, children: isActive ? 'Stop Simulation' : 'Start Simulation' }) }), _jsxs("div", { style: { marginBottom: '16px' }, children: [_jsx("h4", { className: "section-title", children: "Sound Source Type" }), _jsx("div", { className: "sound-type-grid", children: Object.keys(soundPresets).map((type) => (_jsx("button", { onClick: () => setSelectedSourceType(type), className: `sound-type-button ${selectedSourceType === type ? 'selected' : 'unselected'}`, children: type.charAt(0).toUpperCase() + type.slice(1) }, type))) })] }), _jsxs("div", { className: "privacy-section", children: [_jsx("h4", { className: "section-title", children: "Privacy Analysis" }), _jsxs("div", { style: { marginBottom: '12px' }, children: [_jsxs("label", { htmlFor: "privacy-threshold", className: "privacy-threshold-label", children: ["Privacy Threshold: ", privacyThreshold, " dB"] }), _jsx("input", { id: "privacy-threshold", type: "range", min: "20", max: "50", step: "5", value: privacyThreshold, onChange: (e) => setPrivacyThreshold(parseInt(e.target.value)), className: "privacy-threshold-input" })] }), _jsx("div", { style: { marginBottom: '8px' }, children: _jsxs("label", { className: "checkbox-label", children: [_jsx("input", { type: "checkbox", checked: showSoundWaves, onChange: (e) => setShowSoundWaves(e.target.checked) }), "Show Sound Waves"] }) }), _jsx("div", { style: { marginBottom: '8px' }, children: _jsxs("label", { className: "checkbox-label", children: [_jsx("input", { type: "checkbox", checked: showPrivacyZones, onChange: (e) => setShowPrivacyZones(e.target.checked) }), "Show Privacy Zones"] }) }), _jsx("div", { style: { marginBottom: '8px' }, children: _jsxs("label", { className: "checkbox-label", children: [_jsx("input", { type: "checkbox", checked: showLeakagePaths, onChange: (e) => setShowLeakagePaths(e.target.checked) }), "Show Leakage Paths"] }) })] }), _jsxs("div", { className: "instructions-section", children: [_jsx("h4", { className: "section-title", children: "Instructions" }), _jsx("div", { className: "instructions-text", children: isActive ? 'Click to add sound sources and analyze acoustic privacy.' : 'Start simulation to analyze sound privacy.' }), _jsx("div", { className: "color-legend", children: "Color Legend: Red (Poor Privacy), Yellow (Moderate), Green (Good Privacy)" })] }), _jsxs("div", { className: "statistics-section", children: [_jsx("h4", { className: "section-title", children: "Statistics" }), _jsxs("div", { className: "statistics-text", children: [_jsxs("div", { children: ["Sources: ", soundSources.length] }), _jsxs("div", { children: ["Privacy Zones: ", privacyData.privacyZones.length] }), _jsxs("div", { children: ["Leakage Paths: ", privacyData.leakagePaths.length] }), _jsxs("div", { children: ["Overall Privacy: ", privacyData.overallPrivacy.toFixed(1), "%"] })] })] }), _jsxs("div", { className: "control-buttons", children: [_jsx("div", { className: "control-buttons-row", children: _jsx("button", { onClick: clearSoundSources, disabled: !isActive, className: `clear-button ${isActive ? 'enabled' : 'disabled'}`, children: "Clear Sources" }) }), _jsx("div", { className: "footer-text", children: "Acoustic privacy analysis with sound propagation modeling" })] })] }));
};
