...
export class FurnitureManager {
    ...
    searchFurniture(query) {
        ...
        const result = [];
        for (const items of this.furnitureCatalog.values()) {
            result.push(...items.filter(item => item.name.toLowerCase().includes(lowerQuery) ||
     ...
     */
    async placeFurniture(position, rotation) {
        ...
        try {
            ...
            // Check cache first
            let furnitureNode = cacheManager.get('furnitureCache', cacheKey);
            if (!furnitureNode) {
                // Load the model
            ...
            const scale = new Vector3(item.dimensions.width / bounds.width, item.dimensions.height / bounds.height, item.dimensions.depth / bounds.depth);
     ...
     */
    moveFurniture(placedId, newPosition) {
        const placed = this.placedFurniture.get(placedId);
        if (placed) {
     ...
     */
    rotateFurniture(placedId, newRotation) {
        const placed = this.placedFurniture.get(placedId);
        if (placed) {
     ...
     */
    checkClearance(position, dimensions) {
