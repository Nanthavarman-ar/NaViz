...
const WindTunnelSimulation = ({ scene, onWindChange }) => {
    const [windDirection, setWindDirection] = useState(0); // 0 = North, 90 = East
    ...
    const airflowAnalysisRef = useRef([]);
    ...
    const createWindParticles = () => {
        ...
        particleSystem.direction1 = new BABYLON.Vector3(Math.sin(radDirection) * windSpeed, -0.5, Math.cos(radDirection) * windSpeed);
        particleSystem.direction2 = new BABYLON.Vector3(Math.sin(radDirection + 0.2) * windSpeed, 0.5, Math.cos(radDirection + 0.2) * windSpeed);
    ...
    const createWindVectors = () => {
        ...
        for (let x = -gridSize; x <= gridSize; x++) {
            for (let z = -gridSize; z <= gridSize; z++) {
                ...
                const radDirection = (windDirection * Math.PI) / 180;
                const endPoint = new BABYLON.Vector3(startPoint.x + Math.sin(radDirection) * vectorLength, startPoint.y, startPoint.z + Math.cos(radDirection) * vectorLength);
                // Create arrow shaft
    ...
    const analyzeAirflow = () => {
        ...
        openings.forEach(opening => {
            ...
            const radDirection = (windDirection * Math.PI) / 180;
            const offset = new BABYLON.Vector3(Math.sin(radDirection) * 1.5, 0, Math.cos(radDirection) * 1.5);
    ...
    const applyWindForceToMeshes = () => {
        const radDirection = (windDirection * Math.PI) / 180;
        const forceVector = new BABYLON.Vector3(Math.sin(radDirection) * windSpeed, 0, Math.cos(radDirection) * windSpeed);
    ...
    return (_jsxs("div", { className: "wind-tunnel-simulation-container", children: [_jsx("h3", { className: "simulation-title", children: "Wind Tunnel Simulation" }), _jsx("div", { className: "simulation-toggle-container", children: _jsx("button", { onClick: toggleWindSimulation, className: `simulation-toggle-button ${isActive ? 'active' : 'inactive'}`, children: isActive ? 'Stop Simulation' : 'Start Simulation' }) }), _jsxs("div", { className: "wind-presets-container", children: [_jsx("h4", { className: "section-title", children: "Wind Presets" }), _jsx("div", { className: "wind-presets-grid", children: windPresets.map((preset) => (_jsx("button", { onClick: () => applyWindPreset(preset), className: "wind-preset-button", title: `${preset.name}: ${preset.speed} m/s`, children: preset.name }, preset.name))) })] }), _jsxs("div", { className: "wind-controls-container", children: [_jsx("h4", { className: "section-title", children: "Wind Parameters" }), _jsxs("div", { className: "controlGroup", children: [_jsxs("label", { className: "controlLabel", htmlFor: "windDirectionInput", children: ["Direction: ", windDirection, "\u00B0 (", getDirectionName(windDirection), ")"] }), _jsx("input", { id: "windDirectionInput", type: "range", min: "0", max: "360", step: "15", value: windDirection, onChange: (e) => setWindDirection(parseInt(e.target.value)), className: "rangeInput", "aria-label": "Wind Direction" })] }), _jsxs("div", { className: "controlGroup", children: [_jsxs("label", { className: "controlLabel", htmlFor: "windSpeedInput", children: ["Speed: ", windSpeed, " m/s"] }), _jsx("input", { id: "windSpeedInput", type: "range", min: "0", max: "20", step: "0.5", value: windSpeed, onChange: (e) => setWindSpeed(parseFloat(e.target.value)), className: "rangeInput", "aria-label": "Wind Speed" })] }), _jsxs("div", { className: "controlGroup", children: [_jsxs("label", { className: "controlLabel", htmlFor: "turbulenceInput", children: ["Turbulence: ", (turbulence * 100).toFixed(0), "%"] }), _jsx("input", { id: "turbulenceInput", type: "range", min: "0", max: "1", step: "0.1", value: turbulence, onChange: (e) => setTurbulence(parseFloat(e.target.value)), className: "rangeInput", "aria-label": "Turbulence" })] }), _jsxs("div", { className: "controlGroup", children: [_jsxs("label", { className: "controlLabel", htmlFor: "temperatureInput", children: ["Temperature: ", temperature, "\u00B0C"] }), _jsx("input", { id: "temperatureInput", type: "range", min: "-10", max: "40", step: "1", value: temperature, onChange: (e) => setTemperature(parseInt(e.target.value)), className: "rangeInput", "aria-label": "Temperature" })] })] }), _jsx("div", { className: "simulation-info-text", children: "Wind tunnel simulation shows airflow patterns, wind vectors, and building ventilation analysis" })] }));
};
