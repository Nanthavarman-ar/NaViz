...
const AIStructuralAdvisor = ({ scene, isActive, onClose, onAnalysisComplete, onRecommendationApply }) => {
    ...
    const runAnalysis = useCallback(async () => {
        if (!scene || isAnalyzing || !isOnline)
            return;
        setIsAnalyzing(true);
        setAnalysisProgress(0);
        try {
            // Use retry mechanism for network operations
            const result = await retryNetworkOperation(() => performStructuralAnalysis(), {
                maxAttempts: 3,
                baseDelay: 1000,
