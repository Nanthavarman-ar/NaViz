...
export class AccessibilityChecker {
    ...
    createIssue(element, issueType, description, violationAmount) {
    ...
    calculateElementBoundingBox(element) {
        ...
        const min = new Vector3(element.position.x - halfWidth, element.position.y - halfHeight, element.position.z - halfDepth);
        const max = new Vector3(element.position.x + halfWidth, element.position.y + halfHeight, element.position.z + halfDepth);
        return new BoundingBox(min, max);
    }
    calculateSeverity(issueType, violationAmount) {
        if (!violationAmount) {
        ...
        if (violationRatio <= 0.1)
            return 'low'; // ≤10% violation
        if (violationRatio <= 0.25)
            return 'medium'; // 10-25% violation
        return 'high'; // >25% violation
    }
    getStandardValue(issueType) {
    ...
    checkWheelchairPath(element) {
        // Check corridors, hallways, and accessible routes
        if (!['floor', 'wall'].includes(element.type))
            return { issue: null };
        const widthInches = (element.properties?.width ?? 0) * 39.37; // Convert meters to inches
    ...
    checkStairsCompliance(element) {
        if (element.type !== 'floor' || !element.properties?.hasStairs)
            return { issue: null };
    ...
    checkRampCompliance(element) {
        if (element.type !== 'floor' || !element.properties?.isRamp)
            return { issue: null };
        const slopeRatio = element.properties?.slope ?? 0;
        const maxSlope = 1 / 12; // ADA maximum 1:12 slope (8.33%)
        if (slopeRatio <= maxSlope)
            return { issue: null };
    ...
    checkElevatorAccessibility(element) {
        if (element.type !== 'fixture' || !element.properties?.isElevator)
            return { issue: null };
        ...
        if (widthInches >= minWidth && depthInches >= minDepth)
            return { issue: null };
        let violationAmount = 0;
        let description = '';
        if (widthInches < minWidth) {
    ...
    checkWorkspaceAccessibility(element) {
        // Check workspace accessibility for desks, tables, and work surfaces
        if (!['furniture', 'fixture'].includes(element.type) || !element.properties?.isWorkspace)
            return { issue: null };
