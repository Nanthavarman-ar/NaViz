...
const TopographyGenerator = ({ scene, onTerrainGenerated }) => {
    const [selectedPreset, setSelectedPreset] = useState('hills');
    ...
    const noise = (x, y, octaves, roughness) => {
        let value = 0;
        let amplitude = 1;
        let frequency = 0.01;
    ...
    const createTerrainMesh = (heightMap) => {
        const width = heightMap[0].length;
        const height = heightMap.length;
        // Create ground mesh
        const terrain = BABYLON.MeshBuilder.CreateGroundFromHeightMap('terrain', 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==', {
            width: 200,
            height: 200,
            subdivisions: 128,
        ...
        const texture = new BABYLON.RawTexture(textureData, textureSize, textureSize, BABYLON.Engine.TEXTUREFORMAT_RGBA, scene, false, false, BABYLON.Texture.TRILINEAR_SAMPLINGMODE);
        terrainMaterial.diffuseTexture = texture;
        terrain.material = terrainMaterial;
        return terrain;
    };
    // Generate terrain with progress tracking
    const generateTerrain = async (preset) => {
    ...
    const debouncedGenerateTerrain = useCallback(debounce((preset) => {
    ...
    const addVegetation = (terrain, heightMap) => {
    ...
    return (_jsxs("div", { className: "topography-generator", children: [_jsx("h3", { children: "Topography Generator" }), _jsxs("div", { className: "preset-section", children: [_jsx("h4", { children: "Terrain Presets" }), _jsx("div", { className: "preset-grid", children: terrainPresets.map((preset) => (_jsxs("button", { onClick: () => applyPreset(preset), disabled: isGenerating, className: `preset-button ${selectedPreset === preset.id ? 'selected' : ''}`, title: preset.description, "aria-label": `Select terrain preset: ${preset.name}`, children: [_jsx("div", { className: "preset-button-icon", children: preset.icon }), _jsx("div", { children: preset.name })] }, preset.id))) })] }), _jsxs("div", { className: "custom-params", children: [_jsx("h4", { children: "Custom Parameters" }), _jsxs("div", { className: "param-group", children: [_jsxs("label", { htmlFor: "heightScale", className: "param-label", children: ["Height Scale: ", heightScale.toFixed(1), "m"] }), _jsx("input", { id: "heightScale", type: "range", min: "1", max: "20", step: "0.5", value: heightScale, onChange: (e) => setHeightScale(parseFloat(e.target.value)), className: "param-input", "aria-label": "Height Scale" })] }), _jsxs("div", { className: "param-group", children: [_jsxs("label", { htmlFor: "roughness", className: "param-label", children: ["Roughness: ", (roughness * 100).toFixed(0), "%"] }), _jsx("input", { id: "roughness", type: "range", min: "0.1", max: "1.0", step: "0.1", value: roughness, onChange: (e) => setRoughness(parseFloat(e.target.value)), className: "param-input", "aria-label": "Roughness" })] }), _jsxs("div", { className: "param-group", children: [_jsxs("label", { htmlFor: "octaves", className: "param-label", children: ["Detail Level: ", octaves] }), _jsx("input", { id: "octaves", type: "range", min: "1", max: "8", step: "1", value: octaves, onChange: (e) => setOctaves(parseInt(e.target.value)), className: "param-input", "aria-label": "Detail Level" })] })] }), _jsxs("div", { className: "file-upload-section", children: [_jsx("h4", { children: "Import Heightmap" }), _jsx("input", { type: "file", accept: "image/*", onChange: handleFileUpload, className: "file-input", "aria-label": "Import Heightmap file" })] }), isGenerating && (_jsxs("div", { className: "progress-section", children: [_jsxs("div", { className: "progress-text", children: ["Generating terrain... ", generationProgress.toFixed(0), "%"] }), _jsx("div", { className: "progress-bar-container", children: _jsx("div", { className: "progress-bar-fill", style: { width: `${generationProgress}%` } }) })] })), _jsxs("div", { className: "control-buttons", children: [_jsxs("div", { className: "buttons-container", children: [_jsx("button", { onClick: generateCustomTerrain, disabled: isGenerating, className: "generate-button", children: "Generate Terrain" }), _jsx("button", { onClick: clearTerrain, disabled: isGenerating, className: "clear-button", children: "Clear Terrain" })] }), _jsx("div", { className: "footer-text", children: "AI-powered terrain generation creates realistic landscapes for site analysis and visualization" })] })] }));
};
